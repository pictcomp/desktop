...
pair<int, int> findBestMove(char board[3][3]) {
    // Computer is O, which is the MINIMIZING player
    int bestVal = INT_MAX;
    ...
    for (int i = 0; i < 3; i++) {
        for (int j = 0; j < 3; j++) {
            if (board[i][j] == EMPTY) {
                // Try placing O here
                board[i][j] = PLAYER_O;
                int moveVal = minimax(board, 0, true); // Next turn: X (maximizing)
                board[i][j] = EMPTY;

                // Choose move that minimizes X's advantage
                if (moveVal < bestVal) {
                    bestMove = {i, j};
                    bestVal = moveVal;
                }
    ...
    while (true) {
        ...
        if (isGameOver(board)) {
            ...
            if (result == 10)
                ...
                cout << "Computer wins!\n"; // theoretically shouldn't happen right after your move
            else
                cout << "It's a draw!\n";
            break;
        }
        ...
        if (isGameOver(board)) {
            ...
            if (result == -10)
                ...
                cout << "You win!\n"; // theoretically shouldn't happen right after computer's move
            else
                cout << "It's a draw!\n";
            break;
        }
